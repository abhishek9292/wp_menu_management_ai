...
class WPMMAI_Editor {
    ...
    public static function get_instance() {
        ...
        if (self::$instance === null) {            
            ...
            self::$instance = new WPMMAI_Editor();
        }
    ...
    private function __construct() {
        ...
        //$this->lib = WPMMAI_Lib::get_instance();
    }
    // end of __construct()
    ...
    public function get($property_name) {
        ...
        if (!property_exists($this, $property_name)) {
            syslog(LOG_ERR, 'WPMMAI_Editor class does not have such property '. $property_name);
            return null;
        }
    ...
    protected function get_caps_columns_quant() {
        ...
        if ( isset( $_POST['caps_columns_quant'] ) && in_array( $_POST['caps_columns_quant'], array(1,2,3) ) ) {
            ...
            set_site_transient( 'ure_caps_columns_quant', $value, WPMMAI_Lib::TRANSIENT_EXPIRATION );
        } else {
    ...
    protected function init0() {
        ...
        if ( false === $this->caps_readable ) {
            ...
            set_site_transient( 'ure_caps_readable', $this->caps_readable, WPMMAI_Lib::TRANSIENT_EXPIRATION );
        }
        ...
        if ( false === $this->show_deprecated_caps ) {
            ...
            set_site_transient( 'ure_show_deprecated_caps', $this->show_deprecated_caps, WPMMAI_Lib::TRANSIENT_EXPIRATION );
        }
    ...
    protected function prepare_capabilities_to_save() {
        ...
        foreach ( $this->full_capabilities as $cap ) {
            $cap_id_esc = WPMMAI_Capability::escape( $cap['inner'] );
    ...
    protected function is_full_network_synch() {
        ...
        if ( is_network_admin() ) {   // for Pro version
            ...
            $result = defined('WPMMAI_MULTISITE_DIRECT_UPDATE') && WPMMAI_MULTISITE_DIRECT_UPDATE == 1;
        }
    ...
    protected function last_check_before_update() {        
        ...
        $key_capability = WPMMAI_Own_Capabilities::get_key_capability();
        if ( current_user_can( $key_capability ) ) {    
            // current user is an WPMMAI admin
            return true;
        }
        ...
        if ( in_array( $this->current_role, $current_user->roles ) ) {
            // Do not allow to non-admin user without full access to WPMMAI update his own role
            return false;
        }
    ...
    protected function save_roles() {
        ...
        if (empty($this->role_additional_options)) {
            $this->role_additional_options = WPMMAI_Role_Additional_Options::get_instance($this->lib);
        }
    ...
    public function reset_user_roles() {
        ...
        WPMMAI_Own_Capabilities::init_caps();
    ...
    protected function show_editor() {
        ...
        if ( $this->ure_object == 'user' ) {
            $view = new WPMMAI_User_View();
        } else {
            $this->set_current_role();
            $view = new WPMMAI_Role_View();
        ...
        <div class="wrap">
            ...
            <div id="ure_container">                
                <div id="user_role_editor" class="ure-table-cell" >
                    <form id="ure_form" method="post" action="<?php echo WPMMAI_WP_ADMIN_URL . WPMMAI_PARENT . '?page=users-' . WPMMAI_PLUGIN_FILE; ?>" >			
...
<?php
        ...
        WPMMAI_Role_View::output_confirmation_dialog();
?>            
            </div>
        </div>
        <?php

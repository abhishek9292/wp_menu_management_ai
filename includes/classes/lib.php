 ...
 */
class WPMMAI_Lib extends WPMMAI_Base_Lib {

    const TRANSIENT_EXPIRATION = 600;

    protected $wp_default_role = '';
    protected $advert = null;
    protected $bbpress = null; // reference to the WPMMAI_bbPress class instance
    ...
    protected function __construct($options_id) {
        ...
        $this->debug = defined('WPMMAI_DEBUG') && (WPMMAI_DEBUG == 1 || WPMMAI_DEBUG == true);
        $this->get_bbpress();
        $this->upgrade();
    }
...
//            $this->bbpress = new WPMMAI_bbPress();
//        }
    ...
    public static function get_instance($options_id = '') {
        ...
        if (self::$instance === null) {
            if (empty($options_id)) {
                throw new Exception('WPMMAI_Lib::get_instance() - Error: plugin options ID string is required');
            }
            // new static() will work too
            self::$instance = new WPMMAI_Lib($options_id);
        }
    ...
    protected function upgrade() {
        ...
        $wpmmai_version = $this->get_option('wpmmai_version', '0');
        if (version_compare($wpmmai_version, WPMMAI_VERSION, '<')) {
            // put version upgrade stuff here
            $this->put_option('wpmmai_version', WPMMAI_VERSION, true);
        }
    }
    ...
    protected function init_options($options_id) {
        ...
        if ($this->multisite) {
            if (!function_exists('is_plugin_active_for_network')) {    // Be swpmmai the function is defined before trying to use it
                require_once( ABSPATH . '/wp-admin/includes/plugin.php' );
            }
            $this->active_for_network = is_plugin_active_for_network(WPMMAI_PLUGIN_BASE_NAME);
        }
        $current_blog = $wpdb->blogid;
        if ($this->multisite && $current_blog != $this->main_blog_id) {
            if ($this->active_for_network) {   // plugin is active for whole network, so get WPMMAI options from the main blog
    ...
    public function flush_options() {
        ...
        if ($this->multisite && $current_blog !== $this->main_blog_id) {
            if ($this->active_for_network) {   // plugin is active for whole network, so get WPMMAI options from the main blog
                switch_to_blog($this->main_blog_id);  // in order to save WPMMAI options to the main blog
            }
        }
    ...
    public function user_is_admin($user_id = false) {
        ...
        $wpmmai_key_capability = WPMMAI_Own_Capabilities::get_key_capability();
        if (empty($user_id)) {
            $user_id = get_current_user_id();
        }
        $result = user_can($user_id, $wpmmai_key_capability);
    ...
    public function get_built_in_wp_caps() {
        ...
        $caps_groups = WPMMAI_Capabilities_Groups_Manager::get_instance();
    ...
    public function init_full_capabilities($wpmmai_object) {

        $capabilities = WPMMAI_Capabilities::get_instance();
        $full_list = $capabilities->init_full_list($wpmmai_object);
    ...
    public function about() {
        ...
        <strong><?php esc_html_e('Version:', 'user-role-editor'); ?></strong> <?php echo WPMMAI_VERSION; ?><br/><br/>
        <a class="wpmmai_rsb_link" style="background-image:url(<?php echo WPMMAI_PLUGIN_URL . 'images/vladimir.png'; ?>);" target="_blank" href="http://www.shinephp.com/"><?php _e("Author's website", 'user-role-editor'); ?></a><br/>
        <a class="wpmmai_rsb_link" style="background-image:url(<?php echo WPMMAI_PLUGIN_URL . 'images/user-role-editor-icon.png'; ?>);" target="_blank" href="https://www.role-editor.com"><?php _e('Plugin webpage', 'user-role-editor'); ?></a><br/>
        <a class="wpmmai_rsb_link" style="background-image:url(<?php echo WPMMAI_PLUGIN_URL . 'images/user-role-editor-icon.png'; ?>);" target="_blank" href="https://www.role-editor.com/download-plugin"><?php _e('Plugin download', 'user-role-editor'); ?></a><br/>
        <a class="wpmmai_rsb_link" style="background-image:url(<?php echo WPMMAI_PLUGIN_URL . 'images/changelog-icon.png'; ?>);" target="_blank" href="https://www.role-editor.com/changelog"><?php _e('Changelog', 'user-role-editor'); ?></a><br/>
        <a class="wpmmai_rsb_link" style="background-image:url(<?php echo WPMMAI_PLUGIN_URL . 'images/faq-icon.png'; ?>);" target="_blank" href="http://www.shinephp.com/user-role-editor-wordpress-plugin/#faq"><?php _e('FAQ', 'user-role-editor'); ?></a><br/>
        <hr />
    ...
    public function show_admin_role_allowed() {
        ...
        $show_admin_role = ((defined('WPMMAI_SHOW_ADMIN_ROLE') && WPMMAI_SHOW_ADMIN_ROLE == 1) || $show_admin_role == 1) && $this->user_is_admin();
    ...
    public function get_assign_role() {
        ...
        $assign_role = new WPMMAI_Assign_Role();
